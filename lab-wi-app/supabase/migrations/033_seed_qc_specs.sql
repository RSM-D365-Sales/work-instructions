...
--   Adds realistic release-test panels (Appearance/Colour, pH, Osmolality,
...
BEGIN
  ...
  IF v_item IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.qc_tests WHERE reagent_item_id = v_item) THEN
    INSERT INTO public.qc_tests (reagent_item_id, test_order, name, unit, result_type, lower_limit, upper_limit, target, expected_text, method, created_by) VALUES
      (v_item, 0, 'Appearance / Colour', NULL, 'text', NULL, NULL, NULL, 'Clear, colourless solution; free from visible particulates', 'Visual', owner_id),
  ...
  IF v_item IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.qc_tests WHERE reagent_item_id = v_item) THEN
    INSERT INTO public.qc_tests (reagent_item_id, test_order, name, unit, result_type, lower_limit, upper_limit, target, expected_text, method, created_by) VALUES
      (v_item, 0, 'Appearance / Colour', NULL, 'text', NULL, NULL, NULL, 'Clear, colourless solution', 'Visual', owner_id),
  ...
  IF v_item IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.qc_tests WHERE reagent_item_id = v_item) THEN
    INSERT INTO public.qc_tests (reagent_item_id, test_order, name, unit, result_type, lower_limit, upper_limit, target, expected_text, method, created_by) VALUES
      (v_item, 0, 'Appearance / Colour', NULL, 'text', NULL, NULL, NULL, 'Clear, colourless to pale-yellow solution; no precipitate', 'Visual', owner_id),
  ...
  IF v_item IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.qc_tests WHERE reagent_item_id = v_item) THEN
    INSERT INTO public.qc_tests (reagent_item_id, test_order, name, unit, result_type, lower_limit, upper_limit, target, expected_text, method, created_by) VALUES
      (v_item, 0, 'Appearance / Colour', NULL, 'text', NULL, NULL, NULL, 'Clear, colourless solution; no particulates', 'Visual', owner_id),

...
export interface ReagentOrder {
  ...
  /** Set when the order was generated by a standing (recurring) order. */
  standing_order_id?: string | null;
  /** 1-based position within its standing order's series. */
  standing_order_seq?: number | null;
  reagent_item?: ReagentItem;
  lab?: Lab;
  creator?: Profile;
  requester?: Profile;
  items?: ReagentOrderItem[];
  standing_order?: Pick<StandingOrder, 'id' | 'standing_order_number'> | null;
}

// ── Standing (recurring) reagent orders ─────────────────────
// A series is expanded into real reagent_orders the moment it is saved, so it
// is always bounded by an end date or a delivery count.

export type StandingOrderStatus = 'active' | 'completed' | 'cancelled';
export type StandingOrderFrequency = 'weekly' | 'monthly';
export type StandingOrderEndMode = 'date' | 'count';

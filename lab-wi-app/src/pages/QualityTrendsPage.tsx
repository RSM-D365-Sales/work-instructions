...
import { TrendingUp, FlaskConical, Wrench, CheckCircle, X, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {flagTarget && (
        <FlagCalibrationModal
          ...
          scales={scales}
          itemLabel={selectedItem ? `${selectedItem.item_number} · ${selectedItem.product_name}` : ''}
...
function FlagCalibrationModal({
  instrumentLabel, defaultScaleId, scales, itemLabel, onClose,
}: {
  ...
  const [reason, setReason] = useState(
    `"${instrumentLabel}" flagged from Quality Trends${itemLabel ? ` (${itemLabel})` : ''} — results trending against spec.`
  );
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <p className="text-xs text-gray-400">
            The flag appears on the Scales page and notifies administrators. Clearing it there records the calibration date.
          </p>
        </div>
      </div>
    </div>

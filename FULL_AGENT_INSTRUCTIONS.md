# Full Agent Instructions — Uniflow → Rocket Ship Migration

> **Single-file build.** The complete conversion **instructions** followed by the worked
> **few-shot example**, ready to paste into the agent in one go. The instructions come
> first; the worked example at the end seeds the input→output pattern. Assembled from
> `UNIFLOW_TO_ROCKETSHIP_AGENT.md` + `UNIFLOW_AGENT_FEWSHOT.md` — edit those two source
> files and re-assemble; this combined file is a copy for convenience.
...
# Part Two — Worked Example (few-shot)
